-- Manual Affiliate Account Creation System
-- Admin creates affiliate accounts with email, mobile, password
-- Commission is set at product level
...
-- Create affiliate_coupons table (simplified)
...
-- RLS Policies for affiliate_coupons
...
CREATE POLICY "Public can read active coupons" ON public.affiliate_coupons
...
-- Function to create affiliate account with Supabase auth
...
BEGIN
    ...
    -- Note: This needs to be done via Supabase Admin API in the application
    -- For now, we'll create the affiliate account record
...
BEGIN
    ...
    IF NEW.status = 'completed' AND NEW.coupon_code IS NOT NULL THEN
        ...
        IF FOUND THEN
            ...
            INTO product_record
            FROM public.products p
            WHERE p.id = COALESCE(affiliate_record.product_id, NEW.assigned_product_id)
