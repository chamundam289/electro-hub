...
export function ImageUpload({ 
  ...
  const uploadToSupabase = async (file: File): Promise<string | null> => {
    try {
      ...
      console.log('🚀 Attempting direct upload to product-images bucket...');
      
      // Upload file directly to Supabase Storage (skip bucket listing check)
      ...
      if (error.message?.includes('Bucket not found')) {
        toast.error('Storage bucket "product-images" not found. Please ensure it exists in Supabase Dashboard → Storage.');
  ...
  const handleRemove = async () => {
    if (currentImage) {
      try {
        ...
        const filePath = url.pathname.split('/').slice(-2).join('/'); // Get folder/filename
