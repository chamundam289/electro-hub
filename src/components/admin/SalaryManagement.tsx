  ...
  const getAttendanceSummary = async (employeeId: string, month: number, year: number) => {
    try {
      ...
      const summary = {
        ...
        half_days: data?.filter(a => a.status === 'Half Day').length || 0,
        leave_days: data?.filter(a => a.status === 'Leave').length || 0,
        holiday_days: data?.filter(a => a.status === 'Holiday').length || 0,
        total_working_hours: data?.reduce((sum, a) => sum + (a.working_hours || 0), 0) || 0
      };
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900">{salary.employee?.full_name}</h3>
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-green-600">₹{salary.net_salary.toLocaleString()}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <div>
                  ...
                  <p className="font-semibold">{salary.present_days}/{salary.total_working_days}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Gross Salary</p>
                  <p className="font-semibold">₹{salary.gross_salary.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Deductions</p>
                  <p className="font-semibold text-red-600">₹{salary.total_deductions.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Bonus/Incentives</p>
              ...
              <div className="flex gap-2">
                ...
                {salary.payment_status === 'Paid' && salary.payment_mode && (
                  <div className="flex items-center gap-2 px-3 py-1 bg-green-50 rounded text-sm text-green-700">
                    {getPaymentModeIcon(salary.payment_mode)}
                    <span>{salary.payment_mode}</span>
                    {salary.payment_date && (
                      <span>• {format(new Date(salary.payment_date), 'MMM dd')}</span>
                    )}
                  </div>
                )}
              </div>
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <Label htmlFor="salary_year">Year *</Label>
          <Select 
      ...
      <div className="space-y-4">
        <h4 className="font-medium text-gray-900">Additional Components</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 bg-gray-50 rounded-lg">
        <h4 className="font-medium mb-2">{salary.employee?.full_name}</h4>
        <p className="text-2xl font-bold text-green-600">₹{salary.net_salary.toLocaleString()}</p>
        <p className="text-sm text-gray-600">Net Salary Amount</p>
      </div>
