-- Employee Management System Database Setup - FIXED VERSION
...
DROP FUNCTION IF EXISTS set_employee_id() CASCADE;
DROP FUNCTION IF EXISTS calculate_monthly_salary(UUID, INTEGER, INTEGER) CASCADE;
...
-- Function to auto-generate employee ID - FIXED VERSION
...
BEGIN
    -- Get the next sequence number using table alias to avoid ambiguity
    SELECT COALESCE(MAX(CAST(SUBSTRING(e.employee_id FROM 4) AS INTEGER)), 0) + 1
    INTO next_id
    FROM employees e
...
-- Trigger function to auto-generate employee ID - FIXED VERSION
...
-- Function to calculate salary based on attendance - SIMPLIFIED VERSION
...
BEGIN
    ...
    IF emp_record.salary_type = 'Monthly' THEN
        ...
        deduction_amount := (emp_record.base_salary / working_days) * COALESCE(attendance_summary.absent_days, 0);
    ELSIF emp_record.salary_type = 'Daily' THEN
        gross_amount := emp_record.base_salary * (COALESCE(attendance_summary.present_days, 0) + (COALESCE(attendance_summary.half_days, 0) * 0.5));
    ELSIF emp_record.salary_type = 'Hourly' THEN
        gross_amount := emp_record.base_salary * COALESCE(attendance_summary.total_hours, 0);
    END IF;
...
INSERT INTO employees (employee_id, full_name, mobile_number, email, role, department, joining_date, salary_type, base_salary) VALUES
('EMP001', 'John Smith', '+91-9876543210', 'john.smith@company.com', 'Manager', 'Sales', '2024-01-15', 'Monthly', 50000.00),
('EMP002', 'Sarah Johnson', '+91-9876543211', 'sarah.johnson@company.com', 'Sales', 'Sales', '2024-02-01', 'Monthly', 35000.00),
('EMP003', 'Mike Wilson', '+91-9876543212', 'mike.wilson@company.com', 'Technician', 'Technical', '2024-01-20', 'Daily', 1500.00),
('EMP004', 'Lisa Brown', '+91-9876543213', 'lisa.brown@company.com', 'Office Staff', 'Administration', '2024-03-01', 'Monthly', 25000.00);

-- Sample attendance data for current month (last 15 days)
